 ...
 * @return array
 */
function mikrotik_ConfigOptions()
{
    return array(
 ...
 */
function mikrotik_CreateAccount(array $params)
{
    try {
        ...
        $API->debug = false; // Set to true for debugging purposes

        // Get the connection parameters from $params
        $routerIP = 'server ip '; // Router IP
        $routerUsername = 'admin'; // Router Username
        $routerPassword = 'VPNSu8893!'; // Router Password

        $groupName = ''; // Initialize the variable to hold the group name
        $sharedUsers = 0; // Initialize the variable to hold the number of shared users

        // Check the pid value and set the group name and shared-users accordingly
        if ($params['pid'] == 1) {
            $groupName = 'Personal VPN';
            $sharedUsers = 3;
        } elseif ($params['pid'] == 3) {
            $groupName = 'Premium VPN';
            $sharedUsers = 10;
        } elseif ($params['pid'] == 7) {
            $groupName = 'Personal VPN';
            $sharedUsers = 1;
        }else {
            $groupName = 'Unknown Group'; // Optional: Handle other cases if needed
            $sharedUsers = 0; // Default value for unknown cases
        }


        $randomUsername = substr(bin2hex(random_bytes(4)), 0, 8); // 8 characters
        $randomPassword = substr(bin2hex(random_bytes(4)), 0, 8); // 8 characters

        // Command for updating client product
        $command = 'UpdateClientProduct';
        $postData = array(
            'serviceid' => $params['serviceid'],
            'serviceusername' => $params['serviceid'],
            'servicepassword' => $randomPassword
        );
        ...
        if ($API->connect($routerIP, $routerUsername, $routerPassword)) {
            ...
            // Get the user details from $params
            $userName = $params['username'];
            $userPassword = $params['password'];
            // Add the user to the router
            ...
            throw new Exception("Failed to connect to MikroTik router at $routerIP");
        }
 ...
 * @return string "success" or an error message
 */
function mikrotik_SuspendAccount(array $params)
{
    try {
 ...
 * @return string "success" or an error message
 */
function mikrotik_UnsuspendAccount(array $params)
{
    try {
 ...
 * @return string "success" or an error message
 */
function mikrotik_TerminateAccount(array $params)
{
    try {
 ...
 * @return string "success" or an error message
 */
function mikrotik_ChangePassword(array $params)
{
    try {
 ...
 * @return string "success" or an error message
 */
function mikrotik_ChangePackage(array $params)
{
    try {
        // Call the service's change password function, using the values
        // provided by WHMCS in `$params`.
 ...
 * @return string "success" or an error message
 */
function mikrotik_Renew(array $params)
{
    try {
 ...
 * @return array
 */
function mikrotik_TestConnection(array $params)
{
    try {
 ...
 *
 * @see mikrotik_buttonOneFunction()
 *
 * @return array
 */
function mikrotik_AdminCustomButtonArray()
{
    return array(
 ...
 * @return array
 */
function mikrotik_ClientAreaCustomButtonArray()
{
    return array(
 ...
 *
 * @see https://developers.whmcs.com/provisioning-modules/module-parameters/
 * @see mikrotik_AdminCustomButtonArray()
 *
 * @return string "success" or an error message
 */
function mikrotik_buttonOneFunction(array $params)
{
    try {
 ...
 *
 * @see https://developers.whmcs.com/provisioning-modules/module-parameters/
 * @see mikrotik_ClientAreaCustomButtonArray()
 *
 * @return string "success" or an error message
 */
function mikrotik_actionOneFunction(array $params)
{
    try {
        // Call the service's function, using the values provided by WHMCS in
        // `$params`.
 ...
 *
 * @see https://developers.whmcs.com/provisioning-modules/module-parameters/
 * @see mikrotik_AdminServicesTabFieldsSave()
 *
 * @return array
 */
function mikrotik_AdminServicesTabFields(array $params)
{
    try {
        // Call the service's function, using the values provided by WHMCS in
        // `$params`.
        ...
        return array(
            ...
            'Something Editable' => '<input type="hidden" name="mikrotik_original_uniquefieldname" '
                . 'value="' . htmlspecialchars($response['textvalue']) . '" />'
                . '<input type="text" name="mikrotik_uniquefieldname"'
                . 'value="' . htmlspecialchars($response['textvalue']) . '" />',
        );
    } catch (Exception $e) {
        // Record the error in WHMCS's module log.
 ...
 *
 * @see https://developers.whmcs.com/provisioning-modules/module-parameters/
 * @see mikrotik_AdminServicesTabFields()
 */
function mikrotik_AdminServicesTabFieldsSave(array $params)
{
    // Fetch form submission variables.
    $originalFieldValue = isset($_REQUEST['mikrotik_original_uniquefieldname'])
        ? $_REQUEST['mikrotik_original_uniquefieldname']
        : '';

    $newFieldValue = isset($_REQUEST['mikrotik_uniquefieldname'])
        ? $_REQUEST['mikrotik_uniquefieldname']
        : '';
 ...
 * @return array
 */
function mikrotik_ServiceSingleSignOn(array $params)
{
    try {
 ...
 * @return array
 */
function mikrotik_AdminSingleSignOn(array $params)
{
    try {
 ...
 * @return array
 */
function mikrotik_ClientArea(array $params)
{
